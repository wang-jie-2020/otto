# PRD：基于 `npx` 的 Skills 开发与分发体系（非 Marketplace）
...
## 2. 背景与问题
当前目标是用 `npx` 作为技能工具入口，不依赖 ClaudeCode/Codex marketplace 兼容机制。  
