...
1. **SlidePresentation Class** — Main controller with:
   ...
   - Navigation dots
   - **Content overflow detection & auto-fit (CRITICAL)**
...
## Content Auto-Fit (CRITICAL for Naive Design)

**Problem**: Some styles (especially Naive Design with its irregular borders, rotations, and decorative elements) cause content to overflow the slide viewport.

**Solution**: Add `fitSlideContent()` method to SlidePresentation class:
...
class SlidePresentation {
    constructor() {
        ...
        // Auto-fit content after page load
    ...
    /**
     * Detect content overflow and auto-scale to fit.
     * CRITICAL for Naive Design and other dense layouts.
     */
    fitSlideContent() {
        this.slides.forEach((slide, index) => {
            ...
            if (isOverflowing) {
                ...
                console.log(`Slide ${index + 1}: scaled to ${(safeScale * 100).toFixed(0)}%`);
            }
        });
    }
}
...
**Alternative CSS-only approach** (add to slide-content):
